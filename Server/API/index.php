...
$app->get('/sleep/month', function ($request, $response, $args) {
	global $pdo;
	$stt = $pdo->select(array('date', 'label', 'ROUND(time, 2) AS time'))->from('SleepMonth')->orderBy('date', 'DESC')->limit(12)->execute();
	$json = $stt->fetchAll(PDO::FETCH_ASSOC);
...
$app->get('/sleep/week', function ($request, $response, $args) {
	global $pdo;
	$stt = $pdo->select(array('date', 'label', 'ROUND(time, 2) AS time'))->from('SleepWeek')->orderBy('date', 'DESC')->limit(4)->execute();
	$json = $stt->fetchAll(PDO::FETCH_ASSOC);
...
$app->get('/awake/month', function ($request, $response, $args) {
	global $pdo;
	$stt = $pdo->select(array('date', 'label', 'ROUND(time, 2) AS time'))->from('AwakeMonth')->orderBy('date', 'DESC')->limit(12)->execute();
	$json = $stt->fetchAll(PDO::FETCH_ASSOC);
...
$app->get('/awake/week', function ($request, $response, $args) {
	global $pdo;
	$stt = $pdo->select(array('date', 'label', 'ROUND(time, 2) AS time'))->from('AwakeWeek')->orderBy('date', 'DESC')->limit(4)->execute();
	$json = $stt->fetchAll(PDO::FETCH_ASSOC);
